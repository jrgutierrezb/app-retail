...
export class WarehouseRequestComponent implements OnInit {
  ...
  buildForm() {
    return new FormGroup({
      ...
      useridwarehouse:  new FormControl(this.currentUserId, [Validators.required]),
  ...
  save() {
    ...
    let request: IAssetRequest = {
      ...
      useridwarehouse: this.form.get('useridwarehouse').value,
    ...
    this.WareHouseRequest(request).subscribe(
      (respuesta) => {
        if(!respuesta.error) {
  ...
  WareHouseRequest(assetRequest:  IAssetRequest): Observable<BaseResponse> {
    return this.assetRequestService.WareHouseRequest(assetRequest);
  }
